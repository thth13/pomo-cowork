...
function translateTextNode(node: Text, textMap: Record<string, string>, language: string) {
  ...
  const translated = language === defaultLanguage ? source : textMap[key]

  if (translated && node.nodeValue !== withOriginalWhitespace(source, translated)) {
    node.nodeValue = withOriginalWhitespace(source, translated)
  }
}
